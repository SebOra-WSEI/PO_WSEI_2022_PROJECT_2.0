namespace BOOKSTORE_PROJECT_PO.Validators
{
    ...
    public class CustomerValidator : CommonValidator
    {
        ...
        {
            var firstNameResult = this.ValidateInput(firstName, "First Name", 30);
            if (!firstNameResult.IsCorrect)
            {
                return firstNameResult;
            }

            var lastNameResult = this.ValidateInput(lastName, "Last Name", 30);
            if (!lastNameResult.IsCorrect)
            {
                return lastNameResult;
            }

            var emailResult = this.ValidateInput(email, "Email", 50);
